...
export function getItemById(id, items) {
	return items.find((/** @type {{ id: any; }} */ value) => value.id === id);
}

/**
 * @param {any[]} matrix
 * @param {{ w: number; h: number; }} item
 */
export function findFreeSpaceForItem(matrix, item) {
	...
	for (var i = 0; i < getMatrixRows; i++) {
		...
		for (var j = 0; j < xNtime + 1; j++) {
			...
			const empty = sliceA.every((/** @type {undefined} */ val) => val === undefined);
			if (empty) {
				const isEmpty = matrix
					.slice(i, i + item.h)
					.every((/** @type {any[]} */ a) =>
						a.slice(j, j + w).every((/** @type {undefined} */ n) => n === undefined)
					);
...
/**
 * @param {{ [x: string]: any; id: any; }} item
 * @param {number} col
 */
function getItem(item, col) {
	return { ...item[col], id: item.id };
}

/**
 * @param {any[]} elements
 * @param {{ [x: string]: any; id: any; }} active
 * @param {{ y: number; x: number; }} position
 * @param {number} col
 */
function updateItem(elements, active, position, col) {
	return elements.map((/** @type {{ [x: string]: any; id: any; }} */ value) => {
...
export function moveItemsAroundItem(active, items, cols, original) {
	...
	const ids = items
		.map((/** @type {{ id: any; }} */ value) => value.id)
		.filter((/** @type {any} */ value) => value !== activeItem.id);

	const els = items.filter((/** @type {{ id: any; }} */ value) => value.id !== activeItem.id);
	...
	els.forEach((/** @type {{ [x: string]: any; id: any; }} */ item) => {
		...
		let getIgnoreItems = ids.filter((/** @type {any} */ value) => exclude.indexOf(value) === -1);
...
export function moveItem(active, items, cols, original) {
	...
	const fixed = closeObj.find((/** @type {{ [x: string]: string; }} */ value) => value[cols].fixed);
	...
	closeObj.forEach((/** @type {{ [x: string]: any; id: any; }} */ item) => {
		// Find position for element
		let position = findFreeSpaceForItem(matrix, item[cols]);
		// Exclude item
		exclude.push(item.id);
		...
		let getIgnoreItems = tempCloseBlocks.filter(
			(/** @type {any} */ value) => exclude.indexOf(value) === -1
		);
...
export function normalize(items, col) {
	...
	result.forEach((/** @type {{ [x: string]: any; }} */ value) => {
...
export function adjust(items, col) {
	...
	items.forEach((/** @type {{ [x: string]: any; }} */ item) => {
...
export function specifyUndefinedColumns(items, col, breakpoints) {
	...
	getUndefinedElements.forEach((/** @type {any} */ elementId) => {
		const getElement = items.find((/** @type {{ id: any; }} */ item) => item.id === elementId);
